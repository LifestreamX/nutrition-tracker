...
const Articles: React.FC<ArticlesProps> = ({ filteredArticles }) => {
  ...
  const [fullArticles, setFullArticles] = useState(articlesWithIds);

  const handleLike = (id: string) => {
    const updatedArticles = fullArticles.map((article) =>
      article.id === id ? { ...article, likes: article.likes + 1 } : article
    );
    setFullArticles(updatedArticles);
  };
              ...
              <div className='flex w-full justify-end relative top-14 '>
                <div className='flex mx-6 justify-center items-center'>
                  <svg
                    ...
                    strokeWidth={1.5}
                    stroke='currentColor'
                    className='size-6 cursor-pointer mx-1 '
                    width='20'
                    height='20'
